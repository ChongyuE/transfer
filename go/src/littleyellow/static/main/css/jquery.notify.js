...
!function($) {
	...
	var Notify = function(el, options){
		...
		$('<div style="clear:both"></div>').prependTo(this.itemInner);
		$('<div onclick="showNoticeClose()"></div>').addClass('notify-item-close').prependTo(this.itemInner).click(function(){$this.close()});
		//$('<div onclick="showNoticeClose()"></div>').addClass('notify-item-close').prependTo(this.itemInner);
		$('<div style="float:left;line-height: 24px;"><p>来自<span style="color:#FF6C00;font-weight:bold;">硬站站长</span>的消息</p></div>').prependTo(this.itemInner);
